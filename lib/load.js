...
var mwRegExp = /^([a-zA-Z0-9]*)\.middleware\.js$/;
...
var placePath = module.exports.placePath = function(app, path, opts) {
    ...
    files.forEach(function(file) {
        var fpath = join(path, file),
            isMiddleware = mwRegExp.test(file);

        if(isMiddleware) return; // ignore middleware.js files
        ...
        if(stat.isFile()) {
            ...
            hasIndex = fs.existsSync(idxPath);
            hasMW = fs.existsSync(mwPath),
            placing = function() {
